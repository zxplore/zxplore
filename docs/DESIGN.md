...
- **Console** (`zxplore`) — an fzf-driven browser: datasets/snapshots with a
  full properties + permissions dossier, snapshot on tap, point-and-shoot
  replicate, file explore. Plus `zxplore mc` — a pinned-target commander.
- **Transaction API** (`zxplore-api` + `zxplore-txn`) — a scoped
...
chain. Local pool or a remote `host:pool` over SSH (your key/agent; zxplore
